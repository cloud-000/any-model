...
describe("parseTextToolCalls", () => {
    ...
    test('parses format 1: <|tool_call_start|>[get_weather(city="San Francisco")]<|tool_call_end|>', () => {
        ...
        expect(call.args).toEqual({ city: "San Francisco" });
    });

    test("parses format 1 with multiple arguments and types", () => {
        const text =
            '<|tool_call_start|>[get_weather(city="San Francisco", days=5, alert=true)]<|tool_call_end|>';
        const calls = parseTextToolCalls(text);
        expect(calls).toHaveLength(1);
        const call = calls[0]!;
        expect(call.toolName).toBe("get_weather");
        expect(call.args).toEqual({ city: "San Francisco", days: 5, alert: true });
    });

    test('parses format 2: <function name="get_weather"><param name="city">San Francisco</param></function>', () => {
        const text =
            '<function name="get_weather"><param name="city">San Francisco</param></function>';
        const calls = parseTextToolCalls(text);
        expect(calls).toHaveLength(1);
        const call = calls[0]!;
        expect(call.toolName).toBe("get_weather");
    ...
    test("parses format 2 with multiple params and typed values", () => {
        const text =
            '<function name="get_weather"><param name="city">San Francisco</param><param name="days">7</param><param name="detailed">true</param></function>';
        const calls = parseTextToolCalls(text);
        expect(calls).toHaveLength(1);
        const call = calls[0]!;
        expect(call.toolName).toBe("get_weather");
        expect(call.args).toEqual({ city: "San Francisco", days: 7, detailed: true });
    });
});
